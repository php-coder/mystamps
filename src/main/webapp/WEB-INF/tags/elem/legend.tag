	...
	<small>
		<c:if test="${showLinkToAuthPage}">
			<span class="hint_item">
				<spring:message code="t_if_you_already_registered" arguments="${authUrl}" />
			</span>
			<br />
		</c:if>
		<span class="hint_item">
			<spring:message code="t_required_fields_legend" arguments="<span class=\"required_field\">*</span>" />
		</span>
	</small>
</div>
